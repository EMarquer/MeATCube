...
class MeATCubeCB(Generic[SourceSpaceElement, OutcomeSpaceElement]):
    ...
    def increment(self,
        ...
        result_cube = self.add(candidate_cases_sources[result_index], candidate_cases_outcomes[result_index])
        if k > 1:
    ...
    def decrement(self,
        ...
        :return: If `return_all`, returns the updated CB, the competence w.r.t each candidate case, and the index 
            (if `k`==1) or list of indices (if `k`>=1) of the candidate(s) that was added.
        """
        competences = self.competence_contrib(
        ...
        if k > 1:
            ...
            result_cube = self.remove(result_index[0])
            for i in result_index[1:]:
                result_cube = result_cube.remove(i)
        else:
            result_index = candidate_ranking[0]
            result_cube = self.remove(result_index)
